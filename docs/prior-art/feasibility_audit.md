...
| Chronos (Frontiers, 10.3389/fcomp.2026.1873627) | **Yes** — [am-research/rtss-ttft-tbt](https://github.com/am-research/rtss-ttft-tbt), a CPU-only discrete-event simulator *(corrected 2026-09-15: this audit predates the deep-read and said "no code"; see survey/notes/chronos.md and reports/chronos-reproduction.md §2.1)* | N/A | Admission only (WCRT-based) — outer layer OK | ~400–600 | Med | Sporadic-task model + response-time analysis is textbook. Main risk: getting worst-case prefill/decode-iteration bounds calibrated per model. Paper gives numbers for its own harness, not vLLM. |
